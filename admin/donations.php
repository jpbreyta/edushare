...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_donations') {
    ...
    // Fetch total count before search for DataTables metadata
    $totalRecordsStmt = $conn->query("SELECT COUNT(*) as total FROM donations");
    $totalRecords = $totalRecordsStmt ? $totalRecordsStmt->fetch_assoc()['total'] : 0;
    if($totalRecordsStmt) $totalRecordsStmt->close();
